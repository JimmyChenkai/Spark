 ...
 * 实现可选筛选器以减少联接枚举的搜索空间。
 *
 * 1) Star-join filters: PlanStar连接在一起，因为假设它们基于它们的RI关系有一个最佳的执行。
 * 2) Cartesian products: 稍后在图中推迟它们的计划，以避免出现大的中间结果（通常是扩展连接。
 * 3) Composite inners: 不要生成“bushy tree”计划来避免实现中间结果。
 *
 * 2和3还没实现
 */
object JoinReorderDPFilters extends PredicateHelper {
  ...
  /**
   * 应用星型联接筛选器，以消除星型表和非星型表之间的联接组合，直到建立星型联接。
   *
   * 在oneSideJoinPlan/otherSideJoinPlan中, 
   * 它表示由dp join枚举生成的所有计划排列，以及星/非星计划，
   * 允许以下计划组合：
   * 1. (oneSideJoinPlan U otherSideJoinPlan) 是star-join子集
   * 2. star-join 是(oneSideJoinPlan U otherSideJoinPlan)子集
   * 3. (oneSideJoinPlan U otherSideJoinPlan) 是non star-join子集
   *
   * 它假定集合是不相交的。
   *
   * 例子
